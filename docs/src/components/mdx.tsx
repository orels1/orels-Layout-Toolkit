...
type ImagePropsWithOptionalAlt = Omit<ImageProps, 'alt'> & { alt?: string; title?: string }

export const img = function Img(props: ImagePropsWithOptionalAlt) {
  return (
    <>
      <div className="relative mt-8 flex justify-center overflow-hidden rounded-xl bg-gray-50 dark:bg-gray-900 [&+*]:mt-8">
        <Image
          alt=""
      ...
      {props.title && (
        <div className="opacity-70 mb-4 text-center text-sm">{props.title}</div>
      )}
    </>
  )
}
...
function ArticleHeader({ id, date, label }: { id: string; date: string | Date; label?: string; }) {
  return (
      ...
      <div className="pointer-events-none absolute left-[max(-0.5rem,calc(50%-18.625rem))] top-0 z-50 flex h-4 items-center justify-end gap-x-2 lg:left-0 lg:right-[calc(max(2rem,50%-38rem)+40rem)] lg:min-w-[32rem] xl:h-8">
        <Link href={`#${id}`} className="inline-flex">
          {!label && date ? (
            ...
            <div className="hidden xl:block text-2xs/4 font-medium text-gray-500 dark:text-white/50">
              {label}
            </div>
          )}
        </Link>
...
export const article = function Article({
  ...
  return (
    ...
    >
      <div ref={heightRef}>
        <ArticleHeader id={id} date={date} label={label} />
